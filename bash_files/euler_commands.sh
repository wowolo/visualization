...
memory="34000" # memory allocation (in MB) per core (large value implies low priority)
...
# submit the job
# bsub -G ls_math -J $tag -o $log -e $err -n $n_core -W $max_time -N -R "rusage[mem=$memory, ngpus_excl_p=$n_gpus]" python $project_path/main.py -f $LSB_JOBINDEX --experimentbatch_name $tag --config_trainer ${config_trainer[@]}
bsub -G ls_math -J $tag -o $log -e $err -n $n_core -W $max_time -N -R "rusage[mem=$memory, ngpus_excl_p=$n_gpus]" sh $project_path/bash_files/main_func.sh
